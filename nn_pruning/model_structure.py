...
import re
from transformers import BertConfig, BartConfig, T5Config, GPT2Config
...
def struct_from_config(config):
    ...
    if config in config2struct:
        structure = config2struct.get(config)
    elif hasattr(config, "config_class"):
